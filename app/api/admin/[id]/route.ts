...
export async function GET(_: Request, context: unknown) {
  ...
  const id =
    typeof (p as Promise<{ id: string }>).then === "function"
      ...
      : (p as { id: string }).id;
  const res = await fetch(`${base}/api/v1/admin/${id}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to fetch admin" }, {
      status: res.status,
    });
  }
...
export async function PATCH(req: Request, context: unknown) {
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to update admin" }, {
      status: res.status,
    });
  }
  return NextResponse.json(data);
...
export async function DELETE(_: Request, context: unknown) {
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to delete admin" }, {
      status: res.status,
    });
  }
  return NextResponse.json(data);
