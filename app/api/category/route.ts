...
export async function GET(req: Request) {
  ...
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to fetch categories" }, { status: res.status });
  }
...
export async function POST(req: Request) {
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to create category" }, { status: res.status });
  }
  return NextResponse.json(data);
}
