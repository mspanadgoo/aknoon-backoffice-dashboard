...
export async function GET(_: Request, context: unknown) {
  ...
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to fetch category" }, { status: res.status });
  }
...
export async function PATCH(req: Request, context: unknown) {
  ...
  const data = await res.json().catch(() => null);
  if (!res.ok) {
    return NextResponse.json(data ?? { error: "Failed to update category" }, { status: res.status });
  }
  return NextResponse.json(data);
}
...
export async function DELETE(_: Request, context: unknown) {
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => null);
    return NextResponse.json(data ?? { error: "Failed to delete category" }, { status: res.status });
  }
